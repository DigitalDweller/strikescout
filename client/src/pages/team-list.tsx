      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2" data-testid="text-page-title">
          ...
          {help?.HelpTrigger?.({
            content: {
              title: "Team list",
              body: <p>All teams at this event. Sort by any column (click header). Use search to find teams by number or name. Colors: blue = Sweep (far above), green = Cooking, yellow = Mid, orange = Bad, red = Bad+.</p>,
            },
          })}
        </h1>
                  ...
                  {filteredTeams.map(team => {
                    ...
                    return (
                      <TableRow
                        ...
                        onClick={() => navigate(`/events/${eventId}/teams/${team.id}`)}
                      >
                        ...
                        <TableCell className={`text-center font-bold text-base tabular-nums ${szrColorVal}`} data-testid={`stat-szr-${team.id}`}>
                          {hasData ? szrVal : <span className="text-muted-foreground/40">—</span>}
                        </TableCell>
                        {hasTbaData && (
                          <TableCell className={`text-center font-bold text-base tabular-nums ${oprColorVal}`} data-testid={`stat-opr-${team.id}`}>
                            {opr != null ? opr.toFixed(1) : <span className="text-muted-foreground/40">-</span>}
                          </TableCell>
                        )}
                        {hasTbaData && (
                          <TableCell className={`text-center font-bold text-base tabular-nums ${seedColorVal}`} data-testid={`stat-seed-${team.id}`}>
                            {seed != null ? `#${seed}` : <span className="text-muted-foreground/40">-</span>}
                          </TableCell>
                        )}
                        <TableCell className={`text-center font-bold text-base tabular-nums ${drColor}`} data-testid={`stat-dr-${team.id}`}>
                          {hasData ? <>{drVal}<span className="text-xs text-muted-foreground">%</span></> : <span className="text-muted-foreground/40">—</span>}
                        </TableCell>
                        <TableCell className={`text-center font-bold text-base tabular-nums ${throughputColor}`} data-testid={`stat-throughput-${team.id}`}>
                          {hasData ? throughputVal : <span className="text-muted-foreground/40">—</span>}
                        </TableCell>
                        <TableCell className={`text-center font-bold text-base tabular-nums ${accuracyColor}`} data-testid={`stat-accuracy-${team.id}`}>
                          {hasData ? <>{accuracyVal}<span className="text-xs text-muted-foreground">%</span></> : <span className="text-muted-foreground/40">—</span>}
                        </TableCell>
                        <TableCell className={`text-center font-bold text-base tabular-nums ${autoColor}`} data-testid={`stat-auto-${team.id}`}>
