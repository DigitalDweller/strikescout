...
import { computeTeamStats, computeStatRanges, computeStatRangesForSzr, computeTbaRanges, computeSzrMapWithSweepBonus, parseSzrWeights, parsePredictorWeights } from "@/lib/team-colors";
  ...
  const predictionAnalysis = useMemo(() => {
    ...
    if (redOprSum != null && blueOprSum != null) {
      ...
      if (redSzrSum != null && blueSzrSum != null) {
        ...
        const oprPct = predictorWeights.oprWeight / 100;
        const szrPct = predictorWeights.szrWeight / 100;
        blendedProb = oprPct * oprProb + szrPct * szrProb;
      }
    ...
    if (redAllianceStats && blueAllianceStats) {
      ...
      if (redOprSum == null || blueOprSum == null) {
        ...
        if (redSzrSum != null && blueSzrSum != null) {
          const totalSzr = redSzrSum + blueSzrSum;
          const szrProb = totalSzr > 0 ? (redSzrSum / totalSzr) * 100 : 50;
          const compositePct = predictorWeights.fallbackCompositeWeight / 100;
          const szrPct = predictorWeights.fallbackSzrWeight / 100;
          fallbackProb = compositePct * fallbackProb + szrPct * szrProb;
        }
    ...
    if (redSzrSum != null && blueSzrSum != null && (predictorWeights.oprWeight > 0 || predictorWeights.szrWeight > 0)) {
      result.keyFactors.push("Win probability blends OPR (TBA) and SZR (scouting) per your settings.");
    }
