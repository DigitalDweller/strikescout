...
export default function Schedule() {
  ...
  const TeamBadge = ({ teamNum, alliance }: { teamNum: number | null; alliance: "red" | "blue" }) => {
    ...
    return (
      <span
        className={`font-bold text-base tabular-nums ${alliance === "red" ? "text-red-500 dark:text-red-400" : "text-blue-500 dark:text-blue-400"}`}
        title={team?.teamName}
      >
        {teamNum}
      </span>
          ...
          <CardContent className="p-0">
            <div className="rounded-md border overflow-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-20 text-sm font-bold tabular-nums">Match</TableHead>
                    <TableHead className="text-sm font-bold tabular-nums">Time</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-red-500 dark:text-red-400">Red 1</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-red-500 dark:text-red-400">Red 2</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-red-500 dark:text-red-400">Red 3</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-blue-500 dark:text-blue-400">Blue 1</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-blue-500 dark:text-blue-400">Blue 2</TableHead>
                    <TableHead className="text-center text-sm font-bold tabular-nums text-blue-500 dark:text-blue-400">Blue 3</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {sortedSchedule.map(match => {
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground font-medium tabular-nums">
                        {match.time ? (() => {
                          // iOS Safari is strict about date parsing. Convert "YYYY-MM-DD HH:mm..."
                          // to ISO-like "YYYY-MM-DDTHH:mm..." when needed.
                          const candidate = /^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}/.test(match.time)
