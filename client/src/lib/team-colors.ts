...
export type TeamStats = {
  ...
  hasDefense: boolean;
  /** True if team has at least one entry with evadedDefense scored. */
  hasEvadedDefense: boolean;
...
/** Match predictor weights: OPR vs SZR blend and composite stats. */
export type PredictorWeights = {
  oprWeight: number;      // 0-100, OPR share when both OPR and SZR available
  szrWeight: number;      // 0-100, SZR share when both available (opr + szr = 100)
  fallbackCompositeWeight: number;  // 0-100, composite share when OPR missing
  fallbackSzrWeight: number;       // 0-100, SZR share when OPR missing (fallbackComposite + fallbackSzr = 100)
  composite: {
    auto: number;
    throughput: number;
    accuracy: number;
...
export const DEFAULT_PREDICTOR_WEIGHTS: PredictorWeights = {
  oprWeight: 50,
  szrWeight: 50,
  fallbackCompositeWeight: 40,
  fallbackSzrWeight: 60,
  composite: {
    auto: 2.0,
    throughput: 1.5,
    accuracy: 0.5,
...
/** Parse predictorWeights from event. Returns DEFAULT_PREDICTOR_WEIGHTS if invalid. */
export function parsePredictorWeights(predictorWeights: unknown): PredictorWeights {
  const def = DEFAULT_PREDICTOR_WEIGHTS;
  if (!predictorWeights) return def;
  let parsed: unknown = predictorWeights;
  if (typeof predictorWeights === "string") {
    try {
      parsed = JSON.parse(predictorWeights);
    } catch {
      return def;
    }
  }
  if (parsed && typeof parsed === "object" && "composite" in parsed) {
    const p = parsed as Record<string, unknown>;
    const c = p.composite as Record<string, unknown> | undefined;
    if (!c || typeof c !== "object") return def;
    return {
      oprWeight: Math.max(0, Math.min(100, Number(p.oprWeight) || 50)),
      szrWeight: Math.max(0, Math.min(100, Number(p.szrWeight) || 50)),
      fallbackCompositeWeight: Math.max(0, Math.min(100, Number(p.fallbackCompositeWeight) || 40)),
      fallbackSzrWeight: Math.max(0, Math.min(100, Number(p.fallbackSzrWeight) || 60)),
      composite: {
        auto: Math.max(0, Number(c.auto) || def.composite.auto),
        throughput: Math.max(0, Number(c.throughput) || def.composite.throughput),
        accuracy: Math.max(0, Number(c.accuracy) || def.composite.accuracy),
        defense: Math.max(0, Number(c.defense) || def.composite.defense),
        climb: Math.max(0, Number(c.climb) || def.composite.climb),
        autoClimb: Math.max(0, Number(c.autoClimb) || def.composite.autoClimb),
      },
    };
  }
  return def;
...
/** Full scout-form tile: background + border color (all sides) from ranking heat. */
export function getScoutFieldShellClass(
  value: number,
  min: number,
  max: number,
  sweepThreshold?: number | null,
): string {
  const bg = getHeatBgOnly(value, min, max, sweepThreshold);
  const border = getHeatBorderOnly(value, min, max, sweepThreshold);
  return [bg, border].filter(Boolean).join(" ") || "";
}
  ...
  for (const team of teams) {
    ...
    if (count === 0) {
      ...
      const defSum = hasDefense
        ...
        : 0;
      const evadedEntries = teamEntries.filter(e => e.evadedDefense != null);
      const hasEvadedDefense = evadedEntries.length > 0;
      const evadedSum = hasEvadedDefense
        ? evadedEntries.reduce((s, e) => s + toPct(e.evadedDefense ?? 0), 0) / evadedEntries.length
        : 0;
  ...
  for (const scouterId of scouterIds) {
    ...
    if (count === 0) {
      ...
      const defSum = hasDefense
        ? defenseEntries.reduce((s, e) => s + toPct(e.defenseRating ?? 0), 0) / defenseEntries.length
        : 0;
      const evadedEntries = scouterEntries.filter((e) => e.evadedDefense != null);
      const hasEvadedDefense = evadedEntries.length > 0;
      const evadedSum = hasEvadedDefense
        ? evadedEntries.reduce((s, e) => s + toPct(e.evadedDefense ?? 0), 0) / evadedEntries.length
        : 0;
...
export function computeStatRanges(teamStats: Map<number, TeamStats>): StatRanges | null {
  ...
  const withEvaded = allStats.filter(s => s.hasEvadedDefense);
  ...
  return {
    ...
    evadedDefense: withEvaded.length > 0 ? rangeWithSweep(withEvaded.map(s => s.avgEvadedDefense), { min: 0, max: 100 }) : { min: 0, max: 100 },
...
export function computeStatRangesForSzr(teamStats: Map<number, TeamStats>): StatRanges | null {
  ...
  return {
    ...
    evadedDefense: withEvaded.length > 0 ? rangePercentile(withEvaded.map(s => s.avgEvadedDefense), { min: 0, max: 100 }) : { min: 0, max: 100 },
...
function getTeamSweepCount(
  ...
  if (hasData && statRanges && stats) {
    ...
    if (stats.hasEvadedDefense) colors.push(getHeatColor(stats.avgEvadedDefense, statRanges.evadedDefense.min, statRanges.evadedDefense.max, statRanges.evadedDefense.sweep));
