...
export function AppUpdatesProvider({ children }: AppUpdatesProviderProps) {
  ...
  useEffect(() => {
    ...
    // Safari/iOS can be picky about cookies on EventSource when the API is a different origin.
    // Use credentials when supported so sessions work consistently across devices.
    let es: EventSource;
    try {
