...
/** Emitted when the selected FRC season changes (all clients should return to the dashboard). */
export const CHANNEL_SEASON = "season";
...
/** Notify all connected clients that the selected season changed. */
export function notifySeasonChanged(): void {
  eventBroadcast.emit(CHANNEL_SEASON);
}
